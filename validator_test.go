...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				v: struct {
					IntSlice    []int    `validate:"max:2"`
					StringSlice []string `validate:"max:-7"`
				}{
					IntSlice:    nil,
					StringSlice: nil,
				},
		...
		},
		{
			...
			args: args{
				v: struct {
					IntSlice1    []int    `validate:"max:2"`
					StringSlice1 []string `validate:"len:3"`
					IntSlice2    []int    `validate:"in:0,1"`
					StringSlice2 []string `validate:"min:2"`
				}{
					IntSlice1:    []int{1, 2, -1, 0},
					StringSlice1: []string{"kek", "lol"},
					IntSlice2:    []int{0, 1, 1, 0},
					StringSlice2: []string{"op", "opa"},
				},
			},
			wantErr: false,
		},
		{
			...
			args: args{
				v: struct {
					...
					IntSlice1:    []int{1, 3, -1, 10},
					StringSlice1: []string{"kek0", "lo", "opa"},
					IntSlice2:    []int{2, 1, 0, -1},
					StringSlice2: []string{"", "1", "12", "123"},
				},
			},
			wantErr: true,
			checkErr: func(err error) bool {
		...
		{
			name: "nested without validation",
			args: args{
				v: struct {
					NestedLegal struct {
						A int    `validate:"in:4"`
